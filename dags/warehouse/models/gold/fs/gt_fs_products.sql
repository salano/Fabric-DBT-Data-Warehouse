{{ config(
    ...
    unique_key='sk_fs_product',
    incremental_strategy='delete+insert',
    schema='gold',
    alias='gt_fs_products',
    post_hook="{{update_statistics(this,'ProductStatistics','sk_fs_product')}}"
) }}


WITH snapshot_table AS (
	SELECT 
        HASHBYTES('SHA2_256',{{ dbt_utils.generate_surrogate_key(['driving.source_businesskey', 'driving.source_system', 'driving.dbt_valid_from']) }}) AS sk_fs_product,
        HASHBYTES('SHA2_256',{{ dbt_utils.generate_surrogate_key(['driving.source_businesskey', 'driving.source_system']) }}) AS sk_fs_product_master,
        ...
        UnitPrice,
        SupplierId
	FROM {{ ref('st_fs_products') }} driving
    WHERE 1=1
    {% if is_incremental() %}
