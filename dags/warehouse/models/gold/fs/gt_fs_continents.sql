{{ config(
    ...
    unique_key='sk_fs_continent',
    incremental_strategy='delete+insert',
    schema='gold',
    alias='gt_fs_continents',
    post_hook="{{update_statistics(this,'ContinentStatistics','sk_fs_continent')}}"
) }}


WITH snapshot_table AS (
	SELECT 
        HASHBYTES('SHA2_256',{{ dbt_utils.generate_surrogate_key(['driving.source_businesskey', 'driving.source_system', 'driving.dbt_valid_from']) }}) AS sk_fs_continent,
        HASHBYTES('SHA2_256',{{ dbt_utils.generate_surrogate_key(['driving.source_businesskey', 'driving.source_system']) }}) AS sk_fs_continent_master,
	...
	FROM {{ ref('st_fs_continents') }} driving
    WHERE 1=1
    {% if is_incremental() %}
